...
final class MachOBinaryHasher {
	...
	interface ExportOrInstallNameVisitor {
		...
		void visitCpuType(int cputype);
		void visitCpuSubType(int cpusubtype);
	}
	...
	private void hash(MachOBlob.MachOImageBlob image, ExportOrInstallNameVisitor visitor) {
		...
		visitor.visitCpuType(image.header().cputype());
		visitor.visitCpuSubType(image.header().cpusubtype());

		try (MachOLoadCommandTable loadcmds = image.loadCommands()) {
			MachOBlob.MachOSymtabCommand symtab = null;
			MachOBlob.MachODysymtabCommand dysymtab = null;
			for (MachOBlob.MachOLoadCommand loadCommand : loadcmds) {
