...
abstract class MachOBlob {
	 ...
	 */
	public static final class MachOArchitectureTable implements Iterable<MachOImageBlob>, AutoCloseable {
		private final MachOUniversalBlob blob;
		private final int nfat_arch;
		private final int fa_entsize;
		private MappedByteBuffer fat = null;
		...
		private ByteBuffer fat() {
			if (fat == null) {
				...
				fat = (MappedByteBuffer) blob.source.mmap(FAT_HEADER_SIZE, (long) fa_entsize * nfat_arch).order(blob.order);
			}
		...
		@Override
		public void close() {
			if (fat != null) MappedBufferUtils.unmap(fat);
		}
	}
	...
	public static abstract class MachOImageBlob extends MachOBlob {
		...
		public final class MachOHeader {
			public int cputype() {
				return MachOImageBlob.this.cputype();
			}

			public int cpusubtype() {
				return MachOImageBlob.this.cpusubtype();
			}
	 ...
	 */
	public static final class MachOLoadCommandTable implements Iterable<MachOLoadCommand>, AutoCloseable {
		private final MachOImageBlob blob;
		private final int ncmds;
		private final long sizeofcmds;
		private MappedByteBuffer lc = null;
		...
		private ByteBuffer lc() {
			if (lc == null) {
				...
				lc = (MappedByteBuffer) blob.source.mmap(blob.headerSize(), sizeofcmds).order(blob.order);
			}
		...
		@Override
		public void close() {
			if (lc != null) MappedBufferUtils.unmap(lc);
		}
	}
